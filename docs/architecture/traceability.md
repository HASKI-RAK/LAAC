...
| **REQ-FN-031** | Compute Analytics from xAPI LRS per CSV v2 Metric   | ✅ Complete (Deprecated) | ComputationModule, MetricProviders                   | `IMetricComputation`, v2 CSV-aligned providers (courses/topics/elements)                   | ADR-002, Section 4.2      |
| **REQ-FN-032** | Compute Analytics from xAPI LRS per CSV v3 Metric   | ❌ Not Started           | ComputationModule, MetricProviders                   | `IMetricComputation`, v3 CSV-aligned providers (courses/course-topics/topic-elements/user) | ADR-002, Section 4.2      |
...
- [x] Completion Verification: Coverage achieved for all v2 metrics (1 of 12 providers implemented as reference)

---

## CSV v3 Metrics to Implementation Mapping (REQ-FN-032)

Current CSV: [docs/resources/LAAC_Learning_Analytics_Requirements.v3.csv](../resources/LAAC_Learning_Analytics_Requirements.v3.csv). Provider IDs must match the CSV `ID` field exactly.

| Order | CSV ID                       | Dashboard Level | Description                                                                     | Inputs                                   | Provider ID                  | Implementation Status |
| ----- | ---------------------------- | --------------- | ------------------------------------------------------------------------------- | ---------------------------------------- | ---------------------------- | --------------------- |
| 1     | courses-scores               | course          | Sum of best-attempt scores per element, per course                              | userId; since/until (optional)           | courses-scores               | ❌ Not Started        |
| 2     | courses-max-scores           | course          | Sum of configured max scores per element, per course                            | userId                                   | courses-max-scores           | ❌ Not Started        |
| 3     | courses-time-spent           | course          | Total time across all attempts of all elements, per course                      | userId; since/until (optional)           | courses-time-spent           | ❌ Not Started        |
| 4     | user-last-elements           | user            | 3 most recently completed elements across all courses                           | userId; since/until (optional)           | user-last-elements           | ❌ Not Started        |
| 5     | course-topics-scores         | topic           | Sum of best-attempt scores per element, per topic within a course               | userId, courseId; since/until (optional) | course-topics-scores         | ❌ Not Started        |
| 6     | course-topics-max-scores     | topic           | Sum of configured max scores per element, per topic within a course             | userId, courseId                         | course-topics-max-scores     | ❌ Not Started        |
| 7     | course-topics-time-spent     | topic           | Total time across all attempts of all elements, per topic within a course       | userId, courseId; since/until (optional) | course-topics-time-spent     | ❌ Not Started        |
| 8     | course-last-elements         | course          | 3 most recently completed elements within a course                              | userId, courseId; since/until (optional) | course-last-elements         | ❌ Not Started        |
| 9     | topic-elements-best-attempts | element         | Best attempt (score, completionStatus, completedAt) for each element in a topic | userId, topicId                          | topic-elements-best-attempts | ❌ Not Started        |
| 10    | topic-elements-max-scores    | element         | Configured max score for each element in a topic                                | userId, topicId                          | topic-elements-max-scores    | ❌ Not Started        |
| 11    | topic-elements-time-spent    | element         | Total time across all attempts, per element in a topic                          | userId, topicId; since/until (optional)  | topic-elements-time-spent    | ❌ Not Started        |
| 12    | topic-last-elements          | topic           | 3 most recently completed elements within a topic                               | userId, topicId; since/until (optional)  | topic-last-elements          | ❌ Not Started        |

### CSV Verification Checkpoint (REQ-FN-032)

- [x] Pre-Implementation: Approve CSV v3 ID-to-provider mapping (table above)
- [ ] During Implementation: Provider files named after CSV `ID` slugs; JSDoc references v3 description
- [ ] During Implementation: Output schema aligns with CSV `Output` column
- [ ] During Implementation: Test data validates raw aggregations per v3 definitions
- [ ] Completion Verification: Coverage achieved for all v3 metrics
...
| ----------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **SG-4-003** (Developers: Analytics Metrics)    | REQ-FN-003, REQ-FN-004, REQ-FN-005, REQ-FN-031, REQ-FN-032, REQ-NF-003, REQ-NF-004 | MetricsModule, ComputationModule, LRSClient                       |
...
- **Total Requirements**: 47 (27 functional + 20 non-functional)
- **Mapped to Architecture**: 47 (100%)
...
| --------------------- | ---------------------------------------------------- | -------------- |
| **MetricsModule**     | REQ-FN-001, 003, 004, 005, 030, 031, 032             | 7 requirements |
| **ComputationModule** | REQ-FN-004, 010, 011, 031, 032, REQ-NF-009, 010      | 7 requirements |
