...
- A `docs/observability.md` (or similar) document exists with:
  ...
  - Example dashboard panels (described textually or with JSON/YAML snippets) for log-centric stacks (e.g., Grafana + Loki/OpenSearch)
  - Example alert rules (log queries or metrics derived from logs) for critical conditions
...
- Example log query/alert definitions are syntactically valid for at least one supported stack
...
- Tool-agnostic approach; examples provided for log aggregation stacks but adaptable to other platforms
