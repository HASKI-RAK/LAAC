...
- A metrics endpoint exists at root level route `/metrics` exposing metrics in Prometheus/OpenMetrics format for scraping by monitoring tools
...
- Introduces endpoint: GET /metrics at root level (publicly accessible for Prometheus scraping)

**Note**: This Prometheus metrics export endpoint is distinct from the analytics catalog at `/api/v1/metrics` (see REQ-FN-003).
